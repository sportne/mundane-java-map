...
final class MbTilesFile {
    private static final int HEADER_BYTES = 100;
    private static final int APPLICATION_ID = 0x4D504258;
    ...
    static Fingerprint preflight(
            String sourceId, Path supplied, MbTilesLimits limits, CancellationToken cancellation) {
        ...
        if (!path.isAbsolute()
                || name == null
                || !name.toString().toLowerCase(Locale.ROOT).endsWith(".mbtiles")
        ...
        try {
            ...
            int applicationId = fields.getInt(68);
            if (applicationId != 0 && applicationId != APPLICATION_ID) {
                throw MbTilesFailures.failure(
                        sourceId,
                        "MBTILES_PROFILE_UNSUPPORTED",
                        "Unsupported MbTiles application identifier",
            ...
            throw MbTilesFailures.failure(
                    ...
                    "MbTiles input could not be inspected",
                    Map.of("reason", "type"),
                    exception);
        }
    }
    ...
    private static io.github.mundanej.map.api.SourceException invalid(
        ...
        return MbTilesFailures.failure(
                sourceId,
                "SQLITE_INPUT_INVALID",
                "MbTiles input failed preflight",
    ...
    private static io.github.mundanej.map.api.SourceException changed(
        ...
        return MbTilesFailures.failure(
                sourceId, "SQLITE_INPUT_CHANGED", "MbTiles input changed", Map.of("phase", phase));
    }
