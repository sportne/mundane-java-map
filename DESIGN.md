...
| G19 | [Module feature-completeness review](design/G19-module-feature-completeness-review.md) | G19-001, G19-002, G19-011, and G19-190 complete; remaining module-organized cards proposed |
...
literal vector rendering, G14-003 implements explicit borrowed-source binding, typed filters,
...
| 2026-07-16 | Retain only the private AWT vector-template cache and remove the screen-plan candidate. | Canonical native-`/tmp` evidence passed every vector threshold, while the bounded screen working set thrashed on pan; deleting the failed partition keeps only complexity with measured value. |
...
| 2026-07-13 | Propose isolating bounded GeoJSON support in one Jackson Core adapter; approval remains pending. | A maintained token stream could substantially reduce generic JSON risk while a byte snapshot, packed index, closed RFC 7946 profile, and non-leaking optional module would preserve deterministic source behavior and the JDK-only Level 1 boundary. |
| 2026-07-18 | Close bounded GeoJSON Native Image support on Linux x86-64 with direct Jackson construction. | One real read/query/render/write/reopen/malformed scenario plus explicit service exclusion proves the optional adapter without reflection metadata, resource scanning, or a broader platform claim. |
...
| 2026-07-25 | Approve isolating GeoPackage and MBTiles in two Linux JVM-only Xerial adapters. | Pinned split classifiers, direct read-only construction, fixed format facades, and reuse of G6 image decoding make the SQLite profiles implementation-ready without leaking JDBC, widening Level 1, or claiming unsupported native platforms. |
...
| G10-002 | Strict 2D RFC 7946 profile, typed flat properties/IDs, bounded byte-indexed source, isolated Jackson Core adapter, stable diagnostics, and five later vertical slices | Reviewed |
...
| G13-002 | Secure bounded StAX reader, literal point symbolizer, first rendering slice, and module creation | Approved |
...
| G14-003 | Explicit transactional source registry, filters, zoom context, ordering, and ownership | Complete |
...
| G15-004 | Exact Natural Earth land provenance, resource manifest, shapefile/CRS map path, visual chart, and cleanup | Approved |
...
| G18-010 | Optional adapter creation, bundled Canvas custom element, snapshot vector scene, local viewport gestures, resize, and lifecycle | Approved |
| G18-011 | Feature-source bindings, explicit CRS operations, multipart identity, query generations, cancellation, reports, and ownership | Approved |
| G18-020 | Existing built-in vector symbol drawing parity and unsupported-symbol diagnostics | Approved |
| G18-021 | Existing portrayal parity and bounded explicit-catalog icon resources | Approved |
| G18-022 | Bounded browser text measurement, label placement, and vector snapshot capture | Approved |
| G18-030 | Validated client events, hits, hover/selection, accessible navigation, pointer capture, and toolkit-neutral tool host | Approved |
| G18-031 | Existing measurement, snap, immutable point edit, history, canonical coordinate, and browser overlay integration | Approved |
| G18-040 | Same-origin bounded RGBA transport, raster/elevation requests, Canvas ImageData, caching, diagnostics, and lifecycle | Approved |
| G18-041 | Explicit bounded horizontal repetition across browser vector, raster, labels, interaction, measurement, and edits | Approved |
| G18-050 | Offline-by-default Spring Boot/Vaadin application shell, responsive controls, diagnostics, and tools | Approved |
