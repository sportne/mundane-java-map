...
- G10-042 is complete. GeoPackage catalogs strict 256-pixel tile matrices and opens one explicit
  ...
  and provenance. The viewer accepts either a feature table or a tile table plus zoom.
- G10-043 is complete. The optional `mundane-map-io-mbtiles-xerial` module reads strict real-table
  MBTiles 1.3 raster containers, exposes bounded detached metadata, converts TMS rows exactly, and
...
- G10-050 through G10-053 are complete: the published JDK-only GPX adapter securely snapshots bounded
  ...
  warnings. Its closed grammar, parser budgets, deterministic lifecycle precedence, security
  ...
  G10-054 and G10-055 are complete: the independently implemented published KML adapter securely
...
- G14-001 through G14-006 are complete: the exact matrix is approved, the locked optional adapter
  ...
  stable bind diagnostics, plus bounded typed paint expressions with exact limits, conversions,
...
- [G17-005](tasks/closed/G17-005-per-source-file-code-coverage.md) adds an 80% instruction floor for each
...
- G19-001, G19-002, G19-010, G19-011, G19-014, and G19-190 are complete. The API now carries ordinate-aware geometry
  and advanced portrayal values; core provides bounded dimension-preserving transforms/clipping,
  Simple Features validity/intersection, explicit canonical repair, and a pinned common-CRS/WKT2
  profile with exact same-datum operations, plus a bounded OGC TileMatrixSet 2.0 model with exact
  WebMercatorQuad/WorldCRS84Quad and legacy XYZ behavior; architecture tests govern
...
- The review found foundational gaps in dimensional/collection geometry, structured portrayal,
  raster warping and labels. Format cards build on
